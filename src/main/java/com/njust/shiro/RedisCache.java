 ...
 * @ Author        :  景色分明
 * @ CreateDate    :  2020/12/28$ 22:32$
 * @ UpdateUser    :
 * @ UpdateDate    :  2020/12/28$ 22:32$
 * @ Version       :  1.0
 */
@Slf4j
public class RedisCache<K,V> implements Cache<K,V> {
    ...
    private String getRedisCacheKey(K token){
        if(null==token){
            return null;
        } else {
            return this.cacheKey + JwtTokenUtil.getUserId(token.toString()); //根据token获取用户id
        }
    }
}
